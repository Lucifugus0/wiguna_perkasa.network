		...
		if(!empty($aid)){
			for($a = 0; $a < count($aid); $a++){
				if(!empty($aid[$a])){
					$iid = $aid[$a];
					$itarif = $atarif[$a];
					$kode_unik = str_replace("C", "", $iid);
					
					$tagihan_sekarang = $koneksi->query("SELECT * FROM tb_tagihan WHERE id_pelanggan = '".$iid."' AND status = 'LS' AND bulan = '".$_POST['bulan']."' AND tahun = '".$_POST['tahun']."'");
					if (mysqli_num_rows($tagihan_sekarang) >= 1) {
						continue;
					} else {
						$sql_simpan = "INSERT INTO tb_tagihan (bulan, tahun, id_pelanggan, tagihan, kode_unik, status) VALUES (
						   '".$_POST['bulan']."',
						   '".$_POST['tahun']."',
						   '$iid',
						   '$itarif',
