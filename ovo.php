...
file_put_contents("ovo.txt", "Step 1 berhasil!");
if ($_GET) {
	file_put_contents("ovo.txt", "Step 2 berhasil!");
	$post_data = $_GET;
	$pkg = @$post_data['pkg'];
	$text = @$post_data['text'];
	
	if ($pkg != 'ovo.id') {
		file_put_contents("ovo.txt", "Step 3 gagal!");
		die("Invalid request!");
	} else {
		file_put_contents("ovo.txt", "Step 3 berhasil!");
		...
		if (!$filter_jumlah) {
			file_put_contents("ovo.txt", "Step 4 gagal!");
			die("Invalid request!");
		}
		
		file_put_contents("ovo.txt", "Step 4 berhasil!");
		...
		file_put_contents("ovo.txt", "Step 5 berhasil!");
		...
		if ($cek_titipan && $cek_titipan->num_rows > 0) {
			...
			file_put_contents("ovo.txt", "Step 6 titipan berhasil!");
		}
	}
} else {
	file_put_contents("ovo.txt", "Step 2 gagal!");
}
