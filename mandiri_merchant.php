...
file_put_contents("mandiri_merchant.txt", "Step 1 berhasil!");
if ($_GET) {
	file_put_contents("mandiri_merchant.txt", "Step 2 berhasil!");
	$post_data = $_GET;
	$pkg = @$post_data['pkg'];
	$text = @$post_data['text'];
	
	if ($pkg != 'id.bmri.livinmerchant') {
		file_put_contents("mandiri_merchant.txt", "Step 3 gagal!");
		die("Invalid request!");
	} else {
		file_put_contents("mandiri_merchant.txt", "Step 3 berhasil!");
		...
		if (!$filter_jumlah) {
			file_put_contents("mandiri_merchant.txt", "Step 4 gagal!");
			die("Invalid request!");
		}
		
		file_put_contents("mandiri_merchant.txt", "Step 4 berhasil!");
		$filter_jumlah = explode(' ', $filter_jumlah)[0];
		$jumlah_dibayar = $filter_jumlah;
		
		$cek_tagihan = $koneksi->query("SELECT * FROM tb_tagihan WHERE tagihan + kode_unik = $jumlah_dibayar AND kode_unik != 0 AND status = 'BL'");
		$data_tagihan = $cek_tagihan->fetch_assoc();
		$tgl_bayar = date("Y-m-d");
		$id_tagihan = $data_tagihan['id_tagihan'];
		$koneksi->query("UPDATE tb_tagihan SET status = 'LS', tgl_bayar = '$tgl_bayar' WHERE id_tagihan = '$id_tagihan'");
		file_put_contents("mandiri_merchant.txt", "Step 5 berhasil!");
		
		$id_pelanggan = $data_tagihan['id_pelanggan'];
		$cek_pelanggan = $koneksi->query("SELECT * FROM tb_pelanggan WHERE id_pelanggan = '$id_pelanggan'");
		...
		if ($cek_titipan && $cek_titipan->num_rows > 0) {
			...
			file_put_contents("mandiri_merchant.txt", "Step 6 titipan berhasil!");
		}
	}
} else {
	file_put_contents("mandiri_merchant.txt", "Step 2 gagal!");
}
